...
import { rawModelPaths } from '@/lib/anatomy/model-format.mjs';
...
type Props={system:SystemId;structures:Structure[];selected:string|null;isolated:boolean;opacity:number;context:boolean;labels:boolean;hiddenIds:ReadonlySet<string>;onSelect:(id:string)=>void;onHideSelected:()=>void;onUndoHide:()=>void;onReady:(system:SystemId)=>void};
...
const Viewer=forwardRef<ViewerHandle,Props>(function Viewer(props,ref){
 ...
 useEffect(()=>{
  ...
  renderer.domElement.setAttribute('aria-label','Interactive 3D anatomy. Click to select a structure. Double-click to select and focus. Alt-click to cycle nearby layers. H hides the selected structure; U undoes the last hide. Drag to rotate, scroll to zoom. Use the structure list for keyboard selection.');renderer.domElement.setAttribute('role','img');renderer.domElement.tabIndex=0;el.appendChild(renderer.domElement);
  ...
  const raycaster=new THREE.Raycaster(),pointer=new THREE.Vector2();let currentHover:Mesh|null=null;
  ...
  const focusMesh=(mesh:Mesh)=>{if(mesh.visible&&mesh.parent?.visible)fit(new THREE.Box3().setFromObject(mesh),camera.position.clone().sub(controls.target))};
  const focus=()=>{const m=meshes.get(state.current.selected||'');if(m)focusMesh(m)};
  ...
  const update=()=>{
   ...
   const selectedMesh=p.selected&&!p.hiddenIds.has(p.selected)?meshes.get(p.selected):undefined;
   ...
   for(const [id,mesh]of meshes){const isSelected=id===p.selected;const sys=mesh.userData.system as SystemId;const isContext=sys!==p.system;
    const fascia=!!(mesh.userData.fascia&&!isSelected&&sys===p.system);
    const covering=fascia||!!(selectedBounds&&selectedVolume&&!isSelected&&sys===p.system&&overlaps(mesh.userData.bounds as number[],selectedBounds)&&mesh.userData.volume>selectedVolume*1.15);
    mesh.visible=!p.hiddenIds.has(id)&&(!p.isolated||isSelected)&&(!isContext||p.context);
    mesh.userData.pickThrough=covering;
    ...
    mesh.material.opacity=isContext?.12:fascia?Math.min(p.opacity/100,.10):covering?Math.min(p.opacity/100,.22):p.opacity/100;mesh.material.transparent=mesh.material.opacity<1;mesh.material.depthWrite=mesh.material.opacity>.5;mesh.renderOrder=isContext?0:isSelected?2:1;
   }
  };
  const load=async(id:SystemId)=>{
   if(groups.has(id))return;if(pending.has(id))return pending.get(id);
   const task=(async()=>{
    const entries=state.current.structures.filter(s=>s.system===id);
    const expectedBytes=Math.max(...entries.map(s=>s.indexOffset+s.indices*4));
    const request=async(name:string)=>{const response=await fetch(`/models/${name}`,{signal:abort.signal});if(!response.ok)throw new Error('The model could not be loaded. Check the local server and try again.');return response};
    let buffer:ArrayBuffer;
    if(typeof DecompressionStream!=='undefined'){
     const response=await request(`${id}.mesh`);
     buffer=await new Response(new Blob([await response.arrayBuffer()]).stream().pipeThrough(new DecompressionStream('gzip'))).arrayBuffer();
    }else{
     const parts=await Promise.all(rawModelPaths(id,expectedBytes).map(async name=>(await request(name)).arrayBuffer()));
     const combined=new Uint8Array(expectedBytes);let offset=0;
     for(const part of parts){combined.set(new Uint8Array(part),offset);offset+=part.byteLength;}
     if(offset!==expectedBytes)throw new Error('The model download was incomplete. Retry to reload it.');
     buffer=combined.buffer;
    }
    if(buffer.byteLength!==expectedBytes)throw new Error('The model download was incomplete. Retry to reload it.');
    if(disposed)return;
    ...
    for(const s of entries){
     ...
     const material=new THREE.MeshStandardMaterial({color,roughness:.68,metalness:.03,side:THREE.DoubleSide});const mesh=new THREE.Mesh(geometry,material);mesh.name=s.name;mesh.userData={id:s.id,system:id,color,bounds:s.bounds,volume:volumeOf(s.bounds),fascia:id==='muscular'&&/\bfascia\b/i.test(s.name)};group.add(mesh);meshes.set(s.id,mesh);
    }
  ...
  const hit=(event:MouseEvent,cycle=false)=>{const bounds=renderer.domElement.getBoundingClientRect();pointer.set((event.clientX-bounds.left)/bounds.width*2-1,-(event.clientY-bounds.top)/bounds.height*2+1);raycaster.setFromCamera(pointer,camera);const objects=Array.from(meshes.values()).filter(m=>m.visible&&m.parent?.visible&&m.userData.system===state.current.system);return chooseStructureHit(raycaster.intersectObjects(objects,false),{cycle,selectedId:state.current.selected}) as Mesh|undefined};
  let lastHoverTime=0;
  const interaction=createSelectionInteraction({
   pick:hit,
   select:mesh=>{state.current.onSelect(mesh.userData.id);currentHover=mesh as Mesh;setHover(mesh.name)},
   focus:mesh=>focusMesh(mesh as Mesh),
   start:()=>{tween=null},
  });
  const pointerDown=(e:PointerEvent)=>interaction.down(e);
  const pointerUp=(e:PointerEvent)=>interaction.up(e);
  const pointerCancel=(e:PointerEvent)=>interaction.cancel(e);
  const click=(e:MouseEvent)=>interaction.click(e);
  const doubleClick=(e:MouseEvent)=>{if(interaction.doubleClick(e))e.preventDefault()};
  const pointerMove=(e:PointerEvent)=>{
   if(interaction.move(e))return;
   if(e.timeStamp-lastHoverTime<70)return;lastHoverTime=e.timeStamp;
   const mesh=hit(e);if(mesh!==currentHover){currentHover=mesh||null;setHover(mesh?.name||'');renderer.domElement.style.cursor=mesh?'pointer':'grab'}
  };
  const leave=()=>{setHover('');currentHover=null};
  const keydown=(e:KeyboardEvent)=>{if(e.key==='+'||e.key==='='){e.preventDefault();api.current?.zoom(.85)}if(e.key==='-'){e.preventDefault();api.current?.zoom(1.15)}if(e.key==='0'){e.preventDefault();reset()}if(!e.ctrlKey&&!e.metaKey&&!e.altKey&&!e.repeat){if(e.key.toLowerCase()==='h'){e.preventDefault();state.current.onHideSelected();setHover('')}if(e.key.toLowerCase()==='u'){e.preventDefault();state.current.onUndoHide()}}if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();const v=camera.position.clone().sub(controls.target);v.applyAxisAngle(new THREE.Vector3(0,1,0),e.key==='ArrowLeft'?.15:-.15);camera.position.copy(controls.target).add(v)} };
 ...
 useEffect(()=>{api.current?.update();setHover('');if(props.isolated)api.current?.focus()},[props.selected,props.isolated,props.opacity,props.context,props.hiddenIds]);
